...
export default function Header() {
  ...
  return (
    <header className={`w-full top-0 z-50 transition-all duration-300 fixed bg-zinc-800/50 backdrop-blur-2xl ${isScrolled ? 'h-15' : 'h-25'}`}>
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-full">
        <Navigation />
          ...
          <motion.div
            className='md:flex justify-end gap-12 hidden'
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}  
